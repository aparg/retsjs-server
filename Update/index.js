...
const setImages = async (propertyMls, databaseDirectoryName) => {
  ...
  createConsoleLog(
    __filename,
    `created ${numBatches} number of image requests`
  );

  while (propertiesProcessed < totalProperties) {
    const batch = propertyMls.slice(
      propertiesProcessed,
      propertiesProcessed + batchSize
    );
    ...
    createConsoleLog(
      __filename,
      `processed ${propertiesProcessed} out  of ${totalProperties} requests`
    );
  }
...
const getPropertyUpdates = async (updatePropertyType, updateFromTime) => {
  ...
  const {
    ...
    schemaName,
  } = returnPathForPropertyUpdates(updatePropertyType);
  ...
  const propertyDataApi = await getPropertyData(
    updatePropertyType,
    timeStampSql
  );

  const propertyUpdatePath = path.join(
    __dirname,
    `../Data/${databaseDirectoryName}/${updatePropertyType}Updates.xml`
  );
  ...
  const { propertyData, propertyMls } = await parsePropertyDataXml(
    propertyDataApi.data,
    initialXmlObject(),
    updatePropertyType
  );
  ...
  for (const property of propertyData) {
    const propertyExists = await checkIfPropertyExists(
      property.MLS,
      databasePath,
      tableName
    );
    if (propertyExists) {
      ...
      if (property.PixUpdtedDt !== lastPixUpdateDate) {
        ...
        createConsoleLog(
          __filename,
          `picture updated for existing property ${property.MLS}`
        );
      } else {
        propertiesToBeUpdated.add(property.MLS);
        createConsoleLog(
          __filename,
          `property exists but no pix update ${property.MLS}`
        );
      }
    } else {
      imagesToBeDownloaded.push(property.MLS);
  ...
  const downloadImagesAndTheirNames = await setImages(
  ...
  for (const property of propertyData) {
    if (propertiesToBeCreated.has(property.MLS)) {
      createPropertyFunction(
        property,
        databasePath,
        downloadImagesAndTheirNames[property.MLS] || [],
        tableName,
        clauseCollection
      );
    } else if (propertiesToBeUpdatedWithImages.has(property.MLS)) {
      await updatePropertyWithImagesFunction(
        property,
        downloadImagesAndTheirNames[property.MLS],
        databasePath,
        tableName,
...
const updatePropertyTypeMain = async (updatePropertyType, updateFromTime) => {
  ...
  createConsoleLog(
    __filename,
    `initialized ${updatePropertyType.length} property types updated in ${updateFromTime} hours`
  );
  ...
  for (const propertyType of updatePropertyType) {
    ...
    let timeDifferenceForEach =
      (singlePropertyEndTimer - singlePropertyStartTimer) / 1000;
    createConsoleLog(
      __filename,
      `Update completed for ${propertyType} in ${timeDifferenceForEach} seconds`
    );
  }
  ...
  createConsoleLog(
    __filename,
    `Update completed for ${updatePropertyType.length} property types in ${timeDifferenceInSeconds} seconds`
  );
  process.exit();
};
